...
Normalmente, quando você habilita `suspense`, é garantido que `data` será sempre pronto na renderização:
...
No entanto, ao usá-lo junto com busca condicional ou busca dependente, `data` será `undefined` se a solicitação estiver **pausada**:
