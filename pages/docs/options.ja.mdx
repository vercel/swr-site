...
- `isLoading`: フォールバックのためのデータや `keepPreviousData` による以前のデータがなく、`fetcher` から返されるデータがない場合
- `isValidating`: リクエストまたは再検証の読み込みがある場合
- `mutate(data?, options?)`: キャッシュされたデータを更新する関数 [（ 詳細 ）](/docs/mutation)

More information can be found [here](/docs/return-values).
...
- `keepPreviousData = false`: 新しいキーに対するデータがロードされるまで以前のキーのデータを返すかどうか  [（詳細）](/docs/return-values)
