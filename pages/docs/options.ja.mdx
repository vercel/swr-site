...
- `initialData`: 返される初期データ（注：フックごとに） [（詳細）](/docs/with-nextjs)
- `revalidateOnMount`: コンポーネントのマウント時に行われる自動再検証を有効または無効にします（デフォルトでは、initialData が設定されていない場合、マウント時に再検証が行われます。このフラグを使用して動作を強制します）
