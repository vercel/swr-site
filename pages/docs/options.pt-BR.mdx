...
- `revalidateIfStale = true`: revalidar automaticamente mesmo que tenha dados obsoletos [(detalhes)](/docs/revalidation#disable-automatic-revalidations)
...
- `shouldRetryOnError = true`: tentar novamente quando o fetcher retorna um erro
...
<Callout emoji="💡">
    ...
    Quando estiver usando uma conexão lenta (2G, {'<='} 70Kbps), <code>errorRetryInterval</code> será 10s, e{' '}
    <code>loadingTimeout</code> será 5s por padrão.
