...
Então você pode importar `useSWR` e comece a usá-lo em qualquer componente funcional:
...
filhos via props (note que nós não tratamos o estado de erro para agora).
...
O código fica mais difícil de manter se adicionarmos mais dependência de dados à página.
...
Apesar que possamos evitar passar props para componentes filhos, isto é, usar [Context](https://reactjs.org/docs/context.html), ainda há o problema de conteúdo dinâmico:
componentes dentro da página podem ser dinâmicos, e o componente principal pode não saber o que dados serão necessários por seus componentes filhos.

O SWR resolve o problema perfeitamente. Com o hook `useUser`, o código pode ser refatorado para:
