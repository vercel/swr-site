...
There're also options such as `refreshWhenHidden` and `refreshWhenOffline`. Both are disabled by default sSWR won't fetch when the webpage is not on screen, or there's no network connection.
