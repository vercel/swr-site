...
Optionally, you can also leverage the `isValidating` property representing whether is fetching data or not.

```jsx
function useUser (id) {
  const { data, error, isValidating } = useSWR(`/api/user/${id}`, fetcher)
