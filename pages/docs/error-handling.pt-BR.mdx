...
O SWR usa o [algoritmo de retentativa exponencial](https://en.wikipedia.org/wiki/Exponential_backoff) para tentar novamente a requisição em caso de erro.
O algoritmo permite a aplicação recuperar rapidamente de erros, mas não gasta muitos recursos tentando novamente muito frequentemente.
...
Você pode sempre obter o objeto de erro dentro do componente reactivamente.
