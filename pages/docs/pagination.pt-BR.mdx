...
By default, `useSWRInfinite` fetches each page data sequentially. That makes sense because `useSWRInfinite` supports APIs based on the returned data, like cursor-based API. However, getting all page data takes a long time when we have many pages. You can fetch all page data in parallel by enabling the `parallel` option.
