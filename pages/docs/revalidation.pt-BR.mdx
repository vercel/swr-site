...
O SWR lhe dará a opção de reobter automaticamente. É **inteligente**, o que significa que a re-obtenção só ocorrerá se o componente associado ao hook estiver **visível em tela**.
...
Existem também opções como `refreshWhenHidden` e `refreshWhenOffline`. Ambas são desabilitados por padrão, então o SWR não fará fetch quando a página não estiver na tela, ou não houver conexão com a internet.
...
Para ter certeza que os dados estão sempre atualizados, o SWR revalidará automaticamente quando a rede for recuperada.
...
Desde a versão 1.0, o SWR fornece um hook `useSWRImmutable` para marcar o recurso como imutável:
...
O `revalidateIfStale` controla se o SWR deve revalidar quando ele é montado e há dados desatualizados.
