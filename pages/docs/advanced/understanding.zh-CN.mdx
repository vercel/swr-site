...
这种模式表示设置了回退数据时，请求数据，并在之后进行重新请求的场景。

![A pattern for fallback](/img/understanding/fallback.svg)

### 改变 Key 值 + 设置回退数据

这种模式表示在设置回退数据时，请求数据，再改变 key 值，并在之后进行重新请求的场景。

![A pattern for key change + fallback](/img/understanding/key-change-fallback.svg)

### 改变 Key 值 + 保留上一次请求的数据 + 设置回退数据

这种模式表示在开启 `keepPreviousData` 选项并设置回退数据时，请求数据，再改变 key 值，并在之后进行重新请求的场景。
...
简而言之，我们可以使用 `isValidating` 来表示每次正在进行的重新请求；使用 `isLoding` 来表示 SWR 正在重新请求中，但目前还没有数据可以展示。

<Callout emoji="📝">
  回退数据（Fallback Data）和上一次请求的数据（Previous Data）不会被认为是“已加载的数据”，因此在设置了回退数据或启用 `keepPreviousData` 选项时，你可能有数据要展示。
</Callout>
...
## 返回上一次请求的数据以获得更好的用户体验

在用户连续操作的情况下进行数据请时，例如输入时实时搜索，保留上一次请求的数据可以极大提升用户体验，`keepPreviousData` 选项可以用于启用该行为。下例是一个简单的用户搜索界面:
...
如果启用了 `keepPreviousData`，即使我们改变了 SWR key 值并以新的 key 值重新开始加载，我们仍然可以保留上一次请求的数据。
