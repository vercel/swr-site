...
这种模式表示设置了预请求数据时，请求数据，并在之后进行重新请求的场景。

![A pattern for fallback](/img/understanding/fallback.svg)

### 改变 Key 值 + 设置预请求数据

这种模式表示在设置预请求数据时，请求数据，再改变 key 值，并在之后进行重新请求的场景。

![A pattern for key change + fallback](/img/understanding/key-change-fallback.svg)

### 改变 Key 值 + 保留之前的数据 + 设置预请求数据

这种模式表示在开启 `keepPreviousData` 选项并设置预请求数据时，请求数据，再改变 key 值，并在之后进行重新请求的场景。
...
<Callout emoji="📝">
  预请求数据（Fallback Data）和之前的数据（Previous Data）不会被认为是“已加载的数据”，因此在设置了预请求数据或启用 `keepPreviousData` 选项时，你可能有数据要展示。
</Callout>
