...
o desempenho do hook `useSWR` ainda é importante. Em uma aplicação complexo, pode haver centenas de chamadas `useSWR` em uma única renderização de página.

O SWR garante que seu aplicativo tenha:

- \_sem solicitações desnecessárias
- sem re-renderizações desnecessárias
- nenhum código desnecessário importado

sem qualquer alteração de código de você.

## Desduplicação

É muito comum reutilizar hook SWR em seu aplicativo. Por exemplo, uma aplicação que renderiza o avatar do usuário atual 5 vezes:
...
Você também pode customizar a função de comparação via a opção [`compare`](/docs/options) se você quiser mudar o comportamento.
...
O pacote SWR é [tree-shakeable](https://webpack.js.org/guides/tree-shaking) e side-effect tree.
