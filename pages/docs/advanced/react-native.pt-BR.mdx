...
Para customizar estes comportamentos, você pode substituir os listeners de `foco` e `online` do navegador padrão por listeners de foco e online do React Native e configurar SWR para usá-los.
...
Ao usar `initFocus` e `initReconnect`, também é necessário configurar um [provedor de cache personalizado](/docs/advanced/cache). Você pode usar um `Map()` vazio ou qualquer armazenamento que preferir.
...
<SWRConfig
  value={{
    ...
    initFocus(callback) {
      /* Registre o listener com seu provedor de estado */
    },
...
Para `initReconnect`, requer algumas bibliotecas de terceiros, como [NetInfo](https://github.com/react-native-netinfo/react-native-netinfo) para assinar o status da rede. A implementação será semelhante ao exemplo acima: recebendo uma função `callback` e acionando-a quando a rede se recuperar do offline, o SWR pode iniciar uma revalidação para manter seus dados atualizados.
