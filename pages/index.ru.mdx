...
В этом примере, хук `useSWR` принемает строку `key` и функцию `fetcher`. `key` — это уникальный идентификатор данных (обычно URL-адрес API),
...
SWR создана той же командой, что стоит за [Next.js](https://nextjs.org), фреймворк React.  
Следите за [@vercel](https://twitter.com/vercel) на Твиттере, чтобы получать новости о проекте.

Не стесняйтесь присоединяться к [обсуждениям на GitHub](https://github.com/vercel/swr/discussions)!
