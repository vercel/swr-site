...
Esta nova API de provedor de cache também é mais compatível com renderização simultânea do React 18. Se você estiver adicionando um provedor de cache, certifique-se de usar a função global `mutate` retornada de `useSWRConfig()`.

Você pode ler a documentação dos [provedores de cache](/docs/advanced/cache) para obter mais detalhes.

### useSWRConfig()

Há uma nova API Hook para retornar todas as configurações globais, incluindo o provedor de cache atual e a função global `mutate`:
...
Se você não estiver usando um provedor de cache, a importação global atual `import { mutate } from 'swr'` ainda funciona.
...
- `import { createCache } from 'swr'` is foi removido; use o novo [Cache Provider API](/docs/advanced/cache) no lugar.
...
Também queremos agradecer a toda a comunidade, nossos [110 colaboradores](https://github.com/vercel/swr/graphs/contributors) (+ [45 docs colaboradores](https://github.com/vercel/swr -site/graphs/contributors)) e todos que nos ajudaram e nos deram feedback!
