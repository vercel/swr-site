...
### Cache Provider Personnalisable [#custom-cache-provider]
...
Cette nouvelle API est aussi plus compatible avec le rendu concurrent de React 18. Si vous ajoutez un cache provider, assurez-vous d’utiliser la fonction `mutate` globale retournée par `useSWRConfig()`.

Vous pouvez lire la documentation [Cache Provider](/docs/advanced/cache) pour plus de détails.

### useSWRConfig() [#useswrconfig]

Il y a un nouveau Hook API pour retourner toutes les configurations globales, incluant le cache provider actuel et la fonction `mutate` globale :
...
- `import { createCache } from ’swr’` est supprimé; utilisez plutôt la nouvelle [API Cache Provider](/docs/advanced/cache).
