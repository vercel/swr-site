...
현재 요청의 상태를 알아내고, 해당하는 UI를 리턴할 수 있습니다.
...
이 패턴을 적용함으로써 명령형 방식으로 데이터를 **가져오는** 것에 대해 잊을 수 있습니다: 요청을 시작, 로딩 상태를 업데이트, 최종 결과를 리턴.
...
모든 부모 컴포넌트들은 데이터나 데이터 전달에 관련된 것들을 알 필요가 없습니다. 그냥 렌더링 할 뿐입니다.
